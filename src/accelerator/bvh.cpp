...
static inline bool GetIntersection(const BBox &bounds, const Ray &ray, const Vec3 &invDir, const uint32_t dirIsNeg[3])
{
...
BVHAccel::BVHAccel(const vector<Reference<Primitive> > &p, uint32_t mp, const string &sm)
{
...
//    PBRT_BVH_STARTED_CONSTRUCTION(this, primitives.size());
    ...
    BVHBuildNode *root = recursiveBuild(buildArena, buildData, 0, primitives.size(), &totalNodes, orderedPrims);
    primitives.swap(orderedPrims);
//    Info("BVH created with %d nodes for %d primitives (%.2f MB)", totalNodes, (int)primitives.size(), float(totalNodes * sizeof(LinearBVHNode))/(1024.f*1024.f));
...
BVHBuildNode *BVHAccel::recursiveBuild(MemoryArena &buildArena, vector<BVHPrimitiveInfo> &buildData, uint32_t start, uint32_t end, uint32_t *totalNodes, vector<Reference<Primitive> > &orderedPrims)
{
//    Assert(start != end);
    (*totalNodes)++;
...
//                    Assert(b >= 0 && b < nBuckets);
        ...
        node->InitInterior(dim, recursiveBuild(buildArena, buildData, start, mid, totalNodes, orderedPrims),
...
bool BVHAccel::GetIntersection(const Ray &ray, Intersection& isect) const
{
    if (!nodes) return false;
//    PBRT_BVH_INTERSECTION_STARTED(const_cast<BVHAccel *>(this), const_cast<Ray *>(&ray));
    bool hit = false;
    Vec3 invDir(1.f / ray.m_direction.x, 1.f / ray.m_direction.y, 1.f / ray.m_direction.z);
    ...
    while (true) {
        ...
        if (::GetIntersection(node->bounds, ray, invDir, dirIsNeg))
        {
            if (node->nPrimitives > 0) {
...
//                    PBRT_BVH_INTERSECTION_PRIMITIVE_TEST(const_cast<Primitive *>(primitives[node->primitivesOffset+i].GetPtr()));
                    if (primitives[node->primitivesOffset+i]->GetIntersection(ray, isect))
                    {
...
bool BVHAccel::IsIntersected(const Ray &ray) const
{
    if (!nodes) return false;
...
//                    PBRT_BVH_INTERSECTIONP_PRIMITIVE_TEST(const_cast<Primitive *>(primitives[node->primitivesOffset + i].GetPtr()));
                    if (primitives[node->primitivesOffset+i]->IsIntersected(ray))
                    {
