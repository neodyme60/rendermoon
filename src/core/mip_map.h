...
#ifndef __RENDERMOON_CORE_MIP_MAP__
#define __RENDERMOON_CORE_MIP_MAP__
...
public:
    ...
    MIPMap(uint32_t xres, uint32_t yres, const T *data, bool doTri = false, float maxAniso = 8.0f, ImageWrap wrapMode = TEXTURE_REPEAT);
    ~MIPMap();
...
private:
    ...
    ResampleWeight *resampleWeights(uint32_t oldres, uint32_t newres)
    {
        ...
        for (uint32_t i = 0; i < newres; ++i)
        {
            ...
            float invSumWts = 1.f / (wt[i].weight[0] + wt[i].weight[1] + wt[i].weight[2] + wt[i].weight[3]);
...
template <typename T>
MIPMap<T>::MIPMap(uint32_t sres, uint32_t tres, const T *img, bool doTri, float maxAniso, ImageWrap wm)
{
    doTrilinear = doTri;
...
//    PBRT_MIPMAP_EWA_FILTER(const_cast<MIPMap<T> *>(this), s, t, ds0, ds1, dt0, dt1, minorLength, majorLength, lod, nLevels);
    float d = lod - ilod;
    T val = (1.f - d) * EWA(ilod,   s, t, ds0, dt0, ds1, dt1) + d  * EWA(ilod+1, s, t, ds0, dt0, ds1, dt1);
//    PBRT_FINISHED_EWA_TEXTURE_LOOKUP();
    return val;
}
...
T MIPMap<T>::EWA(uint32_t level, float s, float t, float ds0, float dt0,
    ...
    for (int it = t0; it <= t1; ++it) {
        ...
        for (int is = s0; is <= s1; ++is) {
            ...
            if (r2 < 1.) {
                float weight = weightLut[min(Float2Int(r2 * WEIGHT_LUT_SIZE), WEIGHT_LUT_SIZE-1)];
