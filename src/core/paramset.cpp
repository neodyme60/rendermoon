...
void ParamSet::AddVector(const string &name, const Vec3 *data, int nItems)
{
    EraseVector(name);
    ADD_PARAM_TYPE(Vec3, vectors);
}
...
/*
void ParamSet::AddBlackbodySpectrum(const string &name, const float *data, int nItems)
{
    EraseSpectrum(name);
...
void ParamSet::AddSampledSpectrum(const string &name, const float *data, int nItems)
{
    EraseSpectrum(name);
    Assert(nItems % 2 == 0);
    nItems /= 2;
...
void ParamSet::AddSampledSpectrumFiles(const string &name, const char **names, int nItems)
{
    EraseSpectrum(name);
    Spectrum *s = new Spectrum[nItems];
    for (int i = 0; i < nItems; ++i) {
...
const Vec3 *ParamSet::FindVector(const string &name, int *nItems) const
{
    LOOKUP_PTR(vectors);
}


Vec3 ParamSet::FindOneVector(const string &name, const Vec3 &d) const
{
    LOOKUP_ONE(vectors);
}
...
string ParamSet::ToString() const
{
    ...
    for (i = 0; i < vectors.size(); ++i) {
        char *bufp = buf;
        *bufp = '\0';
        const Reference <ParamSetItem<Vec3>> &item = vectors[i];
        typeString = "vector ";
        // Print _ParamSetItem_ declaration, determine how many to print
        int nPrint = item->nItems;
        ret += string("\"");
